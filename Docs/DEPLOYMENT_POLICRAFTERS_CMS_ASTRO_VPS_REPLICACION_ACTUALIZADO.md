...
## 24. Automatización de Publish Wagtail -> rebuild/deploy Astro

La automatización de publicación ya quedó **implementada y probada en producción**.

El objetivo es que al publicar contenido en Wagtail, el frontend Astro estático se reconstruya y despliegue automáticamente, sin dar acceso al socket Docker al contenedor Django.
...
/deploy-trigger/astro-rebuild
        ...
        | bind mount
        v
/var/lib/policrafters/deploy-trigger/astro-rebuild
        |
        v
systemd.path
        |
...
systemd service
        |
        v
/usr/local/bin/policrafters-astro-deploy.sh
        |
        +--> debounce
        +--> flock
        +--> npm run build
        ...
        |
        v
policrafters_astro actualizado
        |
        v
trigger eliminado
```

### 24.1. Directorio de trigger en el VPS

Crear en el host:

```bash
mkdir -p /var/lib/policrafters/deploy-trigger
```

El contenedor Wagtail se ejecuta con UID/GID 1000. Configurar:

```bash
chown -R 1000:1000 /var/lib/policrafters/deploy-trigger
chmod 755 /var/lib/policrafters/deploy-trigger
```

En el host el UID/GID `1000:1000` puede aparecer como `ubuntu:ubuntu`; dentro del contenedor corresponde a `wagtail:wagtail`.

### 24.2. Bind mount del trigger en el CMS

En `docker-compose.prod.yml`, dentro del servicio `web`:

```yaml
volumes:
  - static_data:/app/static
  - media_data:/app/media
  - /var/lib/policrafters/deploy-trigger:/deploy-trigger
```

Así, `/deploy-trigger` dentro de Wagtail corresponde a `/var/lib/policrafters/deploy-trigger` en el VPS.

Prueba del montaje:

```bash
docker exec policrafters_cms sh -c 'echo "trigger-test" > /deploy-trigger/test.txt'
cat /var/lib/policrafters/deploy-trigger/test.txt
```

Resultado esperado: `trigger-test`.

### 24.3. Signal `page_published` de Wagtail

Se creó `home/signals.py`:
...
Django solamente crea el trigger. No ejecuta Docker, npm ni el despliegue.

### 24.4. Registro del signal
...
No fue necesario cambiar `"home"` en `INSTALLED_APPS`, ya que se comprobó que Django detecta `home.apps.HomeConfig`.

### 24.5. Verificación del trigger

Después de publicar desde Wagtail:

```bash
ls -la /var/lib/policrafters/deploy-trigger/
stat /var/lib/policrafters/deploy-trigger/astro-rebuild
```

Debe existir `astro-rebuild`.

### 24.6. Script de rebuild/deploy
...
WEB_DIR="/var/www/policrafters_web"
TRIGGER="/var/lib/policrafters/deploy-trigger/astro-rebuild"
LOCK="/var/lock/policrafters-astro-deploy.lock"
LOG="/var/log/policrafters-astro-deploy.log"

exec >> "$LOG" 2>&1

echo "=================================================="
echo "$(date -Is) - Astro deploy requested"

(
    flock -n 9 || {
        echo "$(date -Is) - Another deploy is already running. Exiting."
        exit 0
    }

    echo "$(date -Is) - Lock acquired."
    echo "$(date -Is) - Waiting 10 seconds for publish debounce..."
    sleep 10

    cd "$WEB_DIR"

    echo "$(date -Is) - Starting Astro build..."
    npm run build
    echo "$(date -Is) - Astro build completed."

    echo "$(date -Is) - Deploying policrafters_astro..."
    docker compose up -d --build astro
    echo "$(date -Is) - Astro deployment completed successfully."

    rm -f "$TRIGGER"

    echo "$(date -Is) - Trigger removed."
    echo "$(date -Is) - Deployment finished successfully."
...
```bash
chmod 755 /usr/local/bin/policrafters-astro-deploy.sh
```

El script incorpora `set -euo pipefail`, `flock`, debounce de 10 segundos, logs y eliminación del trigger únicamente después de un deploy satisfactorio.

Log:

```text
/var/log/policrafters-astro-deploy.log
```

Consultar:

```bash
tail -100 /var/log/policrafters-astro-deploy.log
```

En tiempo real:

```bash
tail -f /var/log/policrafters-astro-deploy.log
```

Salir de `tail -f` con `Ctrl+C`; esto no detiene la automatización.

### 24.7. Prueba manual del script

Antes de conectarlo a systemd:
...
```text
Astro deployment completed successfully.
Trigger removed.
...
confirmó el contenedor activo.

### 24.8. Servicio systemd

Archivo:

```text
/etc/systemd/system/policrafters-astro-deploy.service
```
...
```ini
[Unit]
Description=Policrafters Astro rebuild and deploy
After=docker.service network-online.target
Requires=docker.service

[Service]
Type=oneshot
ExecStart=/usr/local/bin/policrafters-astro-deploy.sh
```

Al ser `Type=oneshot`, después de terminar correctamente puede aparecer como `inactive (dead)`. Esto es normal.

### 24.9. Watcher systemd.path

Archivo:

```text
/etc/systemd/system/policrafters-astro-deploy.path
```
...
```ini
[Unit]
Description=Watch for Policrafters Wagtail publish trigger

[Path]
PathExists=/var/lib/policrafters/deploy-trigger/astro-rebuild
Unit=policrafters-astro-deploy.service
...
systemctl enable --now policrafters-astro-deploy.path
```

Comprobar:

```bash
systemctl status policrafters-astro-deploy.path
systemctl is-enabled policrafters-astro-deploy.path
systemctl list-paths | grep policrafters
```

El `.path` debe quedar `active (waiting)` y `enabled`.

### 24.10. Prueba completa automática

1. Entrar a `https://astro.novatierra.cloud/admin/`.
2. Modificar contenido.
3. Pulsar **Publish**.
4. No ejecutar manualmente ningún comando de Astro o Docker.
5. Observar:
...
8. Recargar el sitio público y comprobar el contenido nuevo.

### 24.11. Diagnóstico

```bash
systemctl status policrafters-astro-deploy.path
systemctl status policrafters-astro-deploy.service
tail -100 /var/log/policrafters-astro-deploy.log
journalctl -u policrafters-astro-deploy.service --no-pager -n 100
ls -la /var/lib/policrafters/deploy-trigger/
docker ps --filter name=policrafters_astro
```
...
- Wagtail solamente escribe en `/deploy-trigger`.
- El contenedor Django no tiene acceso a `/var/run/docker.sock`.
- Django no ejecuta `npm` ni `docker`.
- `systemd` y el script de deploy se ejecutan en el host.
- El script utiliza rutas y comandos fijos.
- `flock` evita builds simultáneos.
- El trigger se elimina únicamente después de un deploy satisfactorio.

No montar `/var/run/docker.sock` dentro del contenedor Django para esta funcionalidad.

### 24.13. Estado

```text
[COMPLETADA] Publish Wagtail -> rebuild Astro -> deploy automático
```
...
-   [x] Automatización de Publish implementada y probada.
...
Con esto queda reproducida la instalación funcional actual, incluyendo la
automatización del rebuild/deploy de Astro después de un **Publish** en Wagtail.
